...
- [ ] **S1** — `config/settings.py` — Add HTTPS-only validator on `api_base_url`
- [ ] **S2** — `src/client/base_client.py:270` — Add HTTPS guard in `set_auth_token()`
...
- [ ] **A3** — `src/client/booking_client.py:119` — Replace raw `dict` with `PartialBookingPayload` model on PATCH
...
- [ ] **B1** — `src/utils/data_factory.py:206` — Fix Faker locale `"ar_AA"` → `"ar_EG"`
...
- [ ] **B6** — `tests/functional/test_canary.py` — Remove `CanaryPayload`/`CanaryDates`; use canonical `BookingPayload`
...
| G1 | fix(security): enforce HTTPS on api_base_url and set_auth_token | — | pending |
| G2 | fix(models): add PartialBookingPayload; type PATCH method | — | pending |
| G3 | fix(data): correct ar_AA → ar_EG in data_factory | — | pending |
| G4 | fix(types): lowercase any → Any in test_schema_contracts | — | pending |
...
| G7 | fix(canary): replace CanaryPayload with canonical BookingPayload | — | pending |
